...
{
	StaticsPool = Cinder::Memory::DoubleLinkedPool(sizeof(Renderable), count, false);
	MovablesPool = Cinder::Memory::DoubleLinkedPool(sizeof(Renderable), count, false);
...
void RendererSystem::RenderAllActive()
{
	...
	glUniformMatrix4fv(GetUniformLocation(&MainShader, UNIFORMS::PROJECTION_MATRIX), 1, true, CalcProjection(&Owner->Camera).Elements_1D);
...
void RendererSystem::RenderDebugShapes()
{
	...
	glUniformMatrix4fv(GetUniformLocation(&DebugShader, UNIFORMS::PROJECTION_MATRIX), 1, 1, CalcProjection(&Owner->Camera).Elements_1D);
